...
function HybridLogin() {
  ...
  useEffect(() => {
    const savedData = JSON.parse(localStorage.getItem('hybridBasicInfo'));
  ...
  useEffect(() => {
    ...
    if (
      ...
      localStorage.setItem('hybridBasicInfo', JSON.stringify(hybridBasicInfo));
      console.log('Saved to localStorage:', hybridBasicInfo);
    }
  ...
  const handleNext = () => navigate('/hybrid-electricity'); // Navigate to next page
