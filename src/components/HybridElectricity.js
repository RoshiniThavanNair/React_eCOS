...
function HybridElectricity() {
  ...
  useEffect(() => {
    const savedData1 = JSON.parse(localStorage.getItem('HybridBuildingParams'));
  ...
  useEffect(() => {
    ...
    if (
      ...
      localStorage.setItem('HybridBuildingParams', JSON.stringify(HybridBuildingParams));
      console.log('Saved to localStorage:', HybridBuildingParams);
    }
  }, [
    originalTariff,
    nemRate,
  ...
  const handlePrevious = () => navigate('/hybrid-login'); // Navigate to the previous page
  const handleNext = () => navigate('/hybrid-ess'); // Navigate to the next page (ESS)
...
const generateGraphData = () => {
  ...
  if (!buildingLoadData.length) {
    alert('Building Load file is missing. Please upload a file.');
    return;
  }
  ...
  const chartLabels = filteredBuildingLoadData.map(item => item.datetime);
  const chartData = filteredBuildingLoadData.map(item => item.total);
  ...
  return (
    <div className="page-content">
      ...
      <div className="info-box" style={{ display: activePage === 2 ? 'block' : 'none' }}>
        ...
        <div className="form-row">
          <label className="label-left1">
            Upload Building Load and Location Data
          </label>
        </div>
...
export default HybridElectricity;
